...
///
/// [ReaderTheme.themed] 会使用当前应用 [ColorScheme.primaryContainer]，
/// 其他值直接返回硬编码色。widgets 在渲染阅读区时统一走此函数，
/// 避免 state getter 依赖 `BuildContext`。
Color resolveReaderBackground(ReaderTheme theme, ColorScheme cs) {
  return switch (theme) {
    ReaderTheme.themed => cs.primaryContainer,
...
Color resolveReaderTextColor(ReaderTheme theme, ColorScheme cs) {
  return switch (theme) {
    ReaderTheme.themed => cs.onPrimaryContainer,
