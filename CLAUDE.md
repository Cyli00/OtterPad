...
### Markdown 阅读器 (Reader)
- Markdown 渲染引擎重构：flutter_markdown_plus → markdown_widget (SpanNode 架构)
- 自定义 LaTeX / 图片 / HTML 表格 SpanNode 组件
- 阅读器主题系统迁移至 MarkdownConfig per-element 配置
- 高亮系统重构：InlineSyntax → richTextBuilder 后处理（解决 LaTeX 公式冲突与高亮空隙）
