...
AI 连通性测试、普通回答和流式回答共用 `agent_protocol.dart` 的地址构造。Gemini 由 `chatUrl(baseUrl, modelId: ..., stream: ...)` 一次生成含模型与操作的完整地址，调用方不再追加路径；地址首尾空白和末尾多余斜线统一清理。其他协议保留各自资源路径和已有版本段。此修复不改变配置、密钥或设置界面。
