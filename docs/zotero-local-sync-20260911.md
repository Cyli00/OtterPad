...
云端 API 同步与 Local API 预览共用 `ZoteroItemMapper.toDocument`：条目必须属于文献类型，有非空标题，并至少有作者、出版信息、年份、DOI 或 ISBN 中的一项。网页、笔记、附件、已删除条目以及只有文件名/URL/标签/添加时间的占位条目不再进入文库。不强制 DOI，也不根据是否手工创建判断质量。PDF 附件仍通过合格父条目的 `parentItem` 关联；未下载 PDF 的正常题录可以保留。此过滤作用于之后的导入，不自动删除已经导入的记录。
